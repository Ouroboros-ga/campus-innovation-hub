...
<template>
  <div class="space-y-4">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-lg font-semibold text-highlighted">FAQ 管理</h2>
        <p class="text-sm text-muted">运营 FAQ 创建/编辑/发布，与指南同级</p>
      </div>
    ...
    <div v-else class="overflow-x-auto rounded-lg border border-default bg-default">
      <table class="w-full text-sm">
        <thead class="bg-muted/40 text-xs text-muted">
          <tr>
            <th class="px-3 py-2 text-left font-medium">排序</th>
        ...
        <tbody class="divide-y divide-default">
          <tr v-for="f in faqs" :key="f.id" class="hover:bg-muted/20">
            <td class="px-3 py-2 font-mono text-xs">{{ f.sortOrder }}</td>
            <td class="px-3 py-2 text-xs">{{ faqCategoryLabel[f.category] ?? f.category }}</td>
            <td class="px-3 py-2 max-w-[400px] truncate font-medium">{{ f.question }}</td>
            <td class="px-3 py-2"><UBadge :color="f.publicationState==='PUBLISHED'?'success':'neutral'" variant="soft" size="xs">{{ f.publicationState==='PUBLISHED'?'已发布':'草稿' }}</UBadge></td>
            <td class="px-3 py-2">
          ...
          <tr v-if="!faqs.length"><td colspan="5" class="py-10 text-center text-sm text-muted">暂无 FAQ</td></tr>
        </tbody>
      </table>
    </div>
    <div class="flex justify-between text-xs text-muted">
      <span>共 {{ total }} 条</span>
      <UPagination v-if="total>pageSize" :page="page" :total="total" :items-per-page="pageSize" @update:page="p=>{page=p; load()}" />
    </div>

    <UModal v-model:open="modalOpen" :title="isEdit?'编辑 FAQ':'新建 FAQ'" :ui="{ content: 'sm:max-w-[560px]' }">
      <template #body>
        <div class="space-y-3">
          <UFormField label="分类" required>
            <USelect v-model="form.category" :items="Object.entries(faqCategoryLabel).map(([v,l])=>({label:l,value:v}))" />
          </UFormField>
          <UFormField label="问题" :error="fieldErrors.question" required>
            <UInput v-model="form.question" maxlength="300" placeholder="问题 2-300" />
          </UFormField>
          <UFormField label="答案" :error="fieldErrors.answerMd" required>
            <UTextarea v-model="form.answerMd" :rows="8" placeholder="支持 Markdown 20000" />
          </UFormField>
          <div class="grid grid-cols-2 gap-3">
            <UFormField label="排序" :error="fieldErrors.sortOrder"><UInput v-model.number="form.sortOrder" type="number" :min="0" /></UFormField>
            <UFormField label="置顶"><USwitch v-model="form.isFeatured" /></UFormField>
          </div>
        </div>
      </template>
      <template #footer>
        <div class="flex justify-end gap-2">
          <UButton variant="ghost" color="neutral" @click="modalOpen=false">取消</UButton>
          <UButton :loading="saving" @click="onSave">{{ isEdit?'保存':'创建' }}</UButton>
        </div>
      </template>
    </UModal>
  </div>
