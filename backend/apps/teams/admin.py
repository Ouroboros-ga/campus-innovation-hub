...
class TeamPostAdmin(AuditedAdminMixin, admin.ModelAdmin):
...
class TeamRoleAdmin(AuditedAdminMixin, admin.ModelAdmin):
    list_display = ["name", "team_post", "headcount", "sort_order"]
    raw_id_fields = ["team_post"]


@admin.register(TeamApplication)
class TeamApplicationAdmin(AuditedAdminMixin, admin.ModelAdmin):
