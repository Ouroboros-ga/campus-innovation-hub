...
| FE-006 首页 Hero 与快捷入口 | 已完成并提交 | HomeHero 主信息、QuickEntry 四快捷入口、Hero 外壳与移动端 2x2 实现；组件/路由测试通过；desktop/平板视觉验证无横向溢出；`pnpm check` 全绿（47/47 测试、含生产路由集成测试） | 真机约 390px 视口截图受 headless 环境钳制，见偏差 |
